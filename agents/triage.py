...
PARSERS = {
    ...
    "/var/www/html/nextcloud/data/nextcloud.log":     parse_nextcloud,
}
...
def watch_file(path):
    """
    Tail a log file, surviving log rotation.
    Reopens the file when it is rotated (inode changes) or truncated
    (size drops below our read position) — otherwise the tailer would
    hold a stale handle after logrotate and silently read nothing.
    """
    parser = PARSERS.get(path)
    if not parser:
        return

    reported_missing = False   # only log "unavailable" once, not every retry

    while True:
        try:
            with open(path) as f:
                if reported_missing:
                    reported_missing = False   # file came back; allow future notices
                ...
                while True:
                    ...
                    # No new data — check whether the file rotated/truncated
                    time.sleep(0.5)
                    try:
                        st = os.stat(path)
                    except FileNotFoundError:
                        break                      # gone mid-rotation; reopen
                    if st.st_ino != cur_ino or st.st_size < f.tell():
                        break                      # rotated or truncated; reopen
...
def monitor_file_sizes():
    ...
    Distinguishes real tampering from routine log rotation: rotation
    replaces the file (new inode), which is normal and NOT flagged.
    Only an IN-PLACE shrink (same inode, smaller size) is treated as a
    possible tamper — that's truncation of the live log, which is the
    genuinely suspicious case.
    """
    track = {}   # path -> {"size": int, "ino": int}
    try:
        with open(SIZE_TRACK_PATH) as f:
            raw = json.load(f)
        # migrate old {path: size} format if present
        for k, v in raw.items():
            track[k] = v if isinstance(v, dict) else {"size": v, "ino": None}
    except (FileNotFoundError, json.JSONDecodeError):
        pass
                ...
                if isinstance(prev, dict) and prev.get("ino") is not None:
                    ...
                    if not rotated and cur["size"] < prev.get("size", cur["size"]):
                        write_entry({
                            ...
                            "raw":           "LOG TAMPER SUSPECTED: {} truncated in place {} -> {} bytes".format(
